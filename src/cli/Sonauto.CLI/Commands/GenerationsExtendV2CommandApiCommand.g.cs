...
internal static partial class GenerationsExtendV2CommandApiCommand
{
    ...
    private static Option<global::Sonauto.ExtendV2RequestSide> Side { get; } = new(
        name: @"--side")
    {
    ...
    private static Option<float?> ExtendDuration { get; } = new(
        name: @"--extend-duration")
    {
        Description = @"How many seconds to generate on the chosen side.",
    };

    private static Option<float?> CropDuration { get; } = new(
        name: @"--crop-duration")
    {
...
Continues an existing audio clip by generating before or after it.");
                        ...
                        command.Options.Add(Side);
                        command.Options.Add(ExtendDuration);
                        command.Options.Add(CropDuration);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            ...
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::Sonauto.ExtendV2Request>(
                            parseResult,
                            Input,
                            RequestJson,
                            RequestFile,
                        ...
                        var audioBase64 = CliRuntime.WasSpecified(parseResult, AudioBase64) ? parseResult.GetValue(AudioBase64) : __requestBase is not null ? __requestBase.AudioBase64 : default;
                        var side = parseResult.GetRequiredValue(Side);
                        var extendDuration = CliRuntime.WasSpecified(parseResult, ExtendDuration) ? parseResult.GetValue(ExtendDuration) : __requestBase is not null ? __requestBase.ExtendDuration : default;
                        var cropDuration = CliRuntime.WasSpecified(parseResult, CropDuration) ? parseResult.GetValue(CropDuration) : __requestBase is not null ? __requestBase.CropDuration : default;
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);


                                var response = await client.Generations.ExtendV2Async(
                                    tags: tags,
                                    lyrics: lyrics,
                                    prompt: prompt,
